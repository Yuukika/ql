...
def sign(au):
    ...
    if response.status_code == 200:
        rj = response.json()
        if rj['code'] == 0:
            print(f"用户【{au['name']}】签到结果：签到成功！")
        else:
            print(f"用户【{au['name']}】签到结果：{rj['message']}")
...
if __name__ == '__main__':
    ...
    #     },
    # ]
    # Aus = '3f7d9fcec0984a469a8c336b21654639'.split("#")
    for index, account in enumerate(accounts):
        print(f"{'-' * 20}第{index + 1}个账号{'-' * 20}")
        print(f'用户【{account["name"]}】开始执行任务>>>')
        sign(account)
